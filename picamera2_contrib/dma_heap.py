...
# `videodev2` is not available on all Raspberry Pi OS images.
# We only need its ioctl encoding helpers, so provide local implementations.
# Linux ioctl encoding is defined in asm-generic/ioctl.h.

# ioctl bit widths/shifts
_IOC_NRBITS = 8
_IOC_TYPEBITS = 8
_IOC_SIZEBITS = 14
_IOC_DIRBITS = 2
...
def _ioc_size(struct_type):
    # Accept ctypes.Structure classes or instances.
    return ctypes.sizeof(struct_type)


def _IOW(type_, nr, struct_type):
    return _IOC(_IOC_WRITE, type_, nr, _ioc_size(struct_type))


def _IOWR(type_, nr, struct_type):
    return _IOC(_IOC_READ | _IOC_WRITE, type_, nr, _ioc_size(struct_type))
