...
from picamera2_contrib.previews.gl_helpers import *
...
def _get_qglpicamera2(qt_module: _QT_BINDING):
    ...
    class QGlPicamera2(QWidget):
        ...
        def __init__(self, picam2, parent=None, width=640, height=480, bg_colour=(20, 20, 20),
            ...
            self.camera_notifier = QSocketNotifier(self.picamera2_contrib.notifyme_r,
        ...
        def cleanup(self):
            ...
            # disappeared.
            self.picamera2_contrib.detach_preview()
        ...
        def set_overlay(self, overlay):
            if not self.picamera2_contrib.camera_config:
                raise RuntimeError("Camera must be configured before setting overlay")
            if self.picamera2_contrib.camera_config['buffer_count'] < 2:
        ...
        def repaint(self, completed_request, update_viewport=False):
            ...
            if completed_request and completed_request.request not in self.buffers:
                if self.stop_count != self.picamera2_contrib.stop_count:
                    if self.picamera2_contrib.verbose_console:
                        print("Garbage collect", len(self.buffers), "textures")
                    for (_, buffer) in self.buffers.items():
                        glDeleteTextures(1, [buffer.texture])
                    self.buffers = {}
                    self.stop_count = self.picamera2_contrib.stop_count
        ...
        def handle_requests(self):
            if not self.running:
                return
            self.picamera2_contrib.notifymeread.read()
            self.picamera2_contrib.process_requests(self)

        def recalculate_viewport(self):
            window_w = self.width()
            window_h = self.height()

            stream_map = self.picamera2_contrib.stream_map
            camera_config = self.picamera2_contrib.camera_config
